...
with open(path + "trainD.txt", encoding='utf-8') as f:
...
input_ids_test = np.array([input_ids[i] for i in random_order[int(len(input_ids)*0.8):]])  #0.8以外的
...
text_test = np.array(random_order[int(len(input_ids) * 0.8):])  #储存测试集的句子原id
...
test_data = TensorDataset(torch.LongTensor(input_ids_test), 
                          ...
                          torch.LongTensor(text_test))   #把测试集的原id也加到test_data的讯息中
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained(bert_path,return_dict=False)  # /bert_pretrain/
...
def train(model, device, train_loader, optimizer, epoch):   # 训练模型
    ...
    for batch_idx, (x1, x2, x3, y) in enumerate(train_loader):
        ...
        weightD10=torch.tensor([0.75,0.25])   #加入权重
        loss = F.cross_entropy(y_pred, y.squeeze(), weight=weightD10)  # 得到loss
...
def test(model, device, test_loader):    # 测试模型, 得到测试集评估结果
    ...
    for batch_idx, (x1, x2, x3, y, textId) in enumerate(test_loader): #每次跑一个batch，y是结果，x1,2,3不用管，textId是该句子的原id
                                                                      #x1, x2, x3, y, textId 是上面85行左右test_data的写入顺序
        x1, x2, x3, y = x1.to(device), x2.to(device), x3.to(device), y.to(device)
        with torch.no_grad():
...
best_acc = 0.0 
PATH = 'roberta_modelD10Psy.pth'  # 定义模型保存路径
